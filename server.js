...
/*   Updated: 2022/03/16 18:35:41 by rpinto-r         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
const { PORT, FT_CLIENT_ID, FT_CLIENT_SECRET, FT_AUTH_URL, FT_USER_URL, FMT_BADGE_URL } = process.env;
...
const createBadgeUrl = (label, message, color, options) =>
{
  ...
  if (options && options.nologo == "")
    logo = "";
  if (options && options.nolabel == "")
    label = "";
  return FMT_BADGE_URL.replace("LOGO", logo).replace("LABEL", label).replace("MESSAGE", message).replace("COLOR", color)
}
...
{
  return cachios.post(`${FT_AUTH_URL}`, {
...
// {
//   res.json({ error: "Not Found", message: "Route not found" });
//   next();
// });
...
app.get("/users/:userName/project/:projectSlug", async (request, reply) =>
{
  ...
  try
  {
    ...
    if (result.data.projects_users)
    {
      ...
      const project = projects.find(item => sanitizeProjectSlug(item.project.slug) === projectSlug);
      if (project)
      {
        //console.log(project);
        if (project.final_mark)
          return replyBadge(reply, projectSlug, `${project.final_mark}/100`, "2aae48", request.query)
        else
  ...
  catch (error)
  {
    ...
    if (error.response.status == 404)
      ...
      return replyBadge(reply, "error", "unexpected", "red");
  }
});

app.get("/users/:userName/projects", async (request, reply) =>
{
  console.log(request.params, request.query, PORT, FT_CLIENT_ID, FT_CLIENT_SECRET, FT_AUTH_URL, FT_USER_URL, FMT_BADGE_URL);
  try
  {
    let { userName } = request.params;
    const result = await getUserData(userName)
    reply.json(result.data.projects_users.map(item => sanitizeProjectSlug(item.project.slug)))
  }
  catch (error)
  {
